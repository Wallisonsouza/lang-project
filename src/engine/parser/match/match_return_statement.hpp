...
inline node::ReturnStatementNode *
match_return_statement(CompilationUnit &unit, TokenStream &stream,
                       ExpressionParser &exp) {

  auto tok = stream.peek();
  if (!tok || tok->descriptor->kind != core::token::TokenKind::ReturnKeyword) {
    return nullptr;
  }

...
core::node::StatementNode *Parser::parse_import_statement() {
  ...
  while (true) {
    ...
    path_nodes.push_back(unit.ast.create_node<core::node::IdentifierNode>(name));

    if (!unit.tokens.match(core::token::TokenKind::Dot)) break;
  ...
  if (unit.tokens.match(core::token::TokenKind::AsKeyword)) {
    ...
    if (!id_tok) {
      report_error(DiagnosticCode::ExpectedToken, "expected identifier after 'as'", unit.tokens.peek_slice());
    } else {

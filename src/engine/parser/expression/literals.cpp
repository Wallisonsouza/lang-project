...
core::node::IdentifierNode *Parser::parse_identifier_name() {
  ...
  auto *node = unit.ast.create_node<core::node::IdentifierNode>(
      unit.source.buffer.get_text(token->slice.span));
...
Parser::parse_path_segment(core::node::ExpressionNode *base) {
  ...
  return unit.ast.create_node<parser::node::statement::PathExprNode>(base,
                                                                     field);
}

...
private:
  inline static const std::unordered_map<core::token::TokenGroup, const std::string> group_map = {
      {core::token::TokenGroup::Name, "Name"},         {core::token::TokenGroup::Keyword, "Keyword"},
      {core::token::TokenGroup::Operator, "Operator"}, {core::token::TokenGroup::Punctuation, "Punctuation"},
      {core::token::TokenGroup::Literal, "Literal"},   {core::token::TokenGroup::Error, "Error"},
  };

  inline static const std::unordered_map<core::token::TokenKind, const std::string> kind_map = {
...
#define C_YELLOW "\033[33m"
  ...
  static void print_token(const CompilationUnit &unit, const core::token::Token *token) {
    ...
    Console::debug(C_BLUE, "Token", C_RESET, " {\n", "  ", C_GREEN, "Kind", C_RESET, ": ", kind, "\n", "  ", C_GREEN, "Group", C_RESET, ": ", group, "\n", "  ", C_YELLOW, "Text", C_RESET, ": \"",
                   text, "\"\n", "  Line: ", token->range.begin.line, " | Start: ", token->range.begin.column, " | End: ", token->range.end.column, "\n", "}");
  }
};
} // namespace debug
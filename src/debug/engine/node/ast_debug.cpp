...
void TreeLayout::begin_node(bool is_last) {
  ...
  for (bool alive : ancestors_alive) {
    out << (alive ? "│  " : "   ");
  }
...
void ASTDebug::debug_node(const core::node::Node *node, bool isLast) {
  ...
  case NodeKind::NumberLiteral:
    debug_number_literal(
        static_cast<const parser::node::NumberLiteralNode *>(node));
    break;

  case NodeKind::StringLiteral:
    debug_string_literal(
        static_cast<const parser::node::StringLiteralNode *>(node));
    break;

  case NodeKind::BooleanLiteral:
    debug_bool_literal(
        static_cast<const parser::node::BoolLiteralNode *>(node));
    break;

  case NodeKind::CharLiteral:
    debug_char_literal(
        static_cast<const parser::node::CharLiteralNode *>(node));
    break;

  case NodeKind::NullLiteral:
    debug_null_literal(
        static_cast<const parser::node::NullLiteralNode *>(node));
    break;
  ...
  case NodeKind::Import:
    debug_import(
        static_cast<const parser::node::statement::ImportNode *>(node));
    break;
  ...
  case NodeKind::VariableDeclaration:
    debug_variable_declaration(static_cast<const PatternNode *>(node));
    break;

  case NodeKind::BinaryExpression:
    debug_binary_expression(
        static_cast<const parser::node::BinaryExpressionNode *>(node));
    break;

  case NodeKind::FunctionCall:
    debug_function_call(
        static_cast<const parser::node::FunctionCallNode *>(node));
    break;
  ...
  case NodeKind::PathExpression:
    debug_path_expression(
        static_cast<const parser::node::statement::PathExprNode *>(node));
    break;

  case NodeKind::IfStatement:
    debug_if_statement(
        static_cast<const parser::node::IfStatementNode *>(node));
    break;
  ...
  case NodeKind::BlockStatement:
    debug_block(static_cast<const parser::node::BlockStatementNode *>(node));
    break;

  case NodeKind::IndexAccess:
    debug_index_acess(static_cast<const parser::node::IndexAccessNode *>(node));
    break;

  case NodeKind::ReturnStatement:
    debug_return_statement(
        static_cast<const parser::node::ReturnStatementNode *>(node));
    break;
...
// void ASTDebug::debug_children(const std::vector<const core::node::Node *>
// &children) {
...
void ASTDebug::debug_labeled(const char *label, const core::node::Node *child,
                             bool is_last) {

...
inline void debug_node(const core::node::Node *node, const std::string &prefix, bool isLast) {
  ...
  case NodeKind::BoolLiteral: debug_bool_literal(static_cast<const parser::node::BoolLiteralNode *>(node), prefix, isLast); break;
  ...
  case NodeKind::ImportStatement: debug_import(static_cast<const parser::node::statement::ImportNode *>(node), prefix, isLast); break;

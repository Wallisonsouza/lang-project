...
inline core::ast::IdentifierNode *id(AST &ast, const char *name) { return ast.create_node<core::ast::IdentifierNode>(name); }

inline core::ast::NativeFunctionDeclarationNode *
make_native(core::Module *module, core::ast::IdentifierNode *name, std::vector<core::ast::PatternNode *> params, core::ast::TypeNode *return_type, Value::NativeFunction callback) {
  auto *node = module->ast.create_node<core::ast::NativeFunctionDeclarationNode>(name, std::move(params), return_type, std::move(callback));
...
inline void create_module_math(LanguageContext &context, core::ModuleId parent = core::INVALID_MODULE) {
  ...
  auto make_param = [&](const char *name) { return module->ast.create_node<core::ast::PatternNode>(id(module->ast, name), number_type, nullptr); };

...
make_native(core::Module *module, const std::string &name,
            std::vector<core::node::FunctionParameterNode *> params,
            core::node::TypeNode *return_type, auto callback) {
...
inline void create_module_console(core::ModuleManager &manager) {
  ...
  auto debug_module = manager.create_module("debug");
  auto console_module = manager.create_module("console", debug_module);

  std::vector<core::node::FunctionParameterNode *> params;
  core::node::TypeNode *return_type =
      console_module->module_scope.arena.create<core::node::VoidTypeNode>();

  make_native(
      console_module, "log", params, return_type,
      [](const std::vector<core::node::Node *> &args) -> core::node::Node * {
        for (size_t i = 0; i < args.size(); ++i) {
          auto *arg = args[i];
          if (arg)
            std::cout << arg->to_string_node();
          if (i + 1 < args.size())
            std::cout << " ";
        }
        std::cout << std::endl;
  ...
  make_native(
      console_module, "print", params, return_type,
      [](const std::vector<core::node::Node *> &args) -> core::node::Node * {
        for (size_t i = 0; i < args.size(); ++i) {
          auto *arg = args[i];
          if (arg)
  ...
  make_native(
      console_module, "warn", params, return_type,
      [](const std::vector<core::node::Node *> &args) -> core::node::Node * {
        for (size_t i = 0; i < args.size(); ++i) {
          auto *arg = args[i];
          if (arg)
            std::cerr << arg->to_string_node();
          if (i + 1 < args.size())
            std::cerr << " ";
        }
        std::cerr << std::endl;
  ...
  make_native(
      console_module, "error", params, return_type,
      [](const std::vector<core::node::Node *> &args) -> core::node::Node * {
        for (size_t i = 0; i < args.size(); ++i) {
          auto *arg = args[i];
          if (arg)

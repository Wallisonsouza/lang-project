...
public:
  ...
  core::node::ExpressionNode *parse_primary(CompilationUnit &unit,
    ...
    case core::token::TokenKind::StringLiteral: {
      ...
      result = unit.ast.create_node<node::StringLiteralNode>(str);
      break;
    }
    case core::token::TokenKind::Identifier: {
      auto name = unit.source.buffer.get_text(tok->span);
      result = unit.ast.create_node<node::IdentifierNode>(name);
      break;
    }

    case core::token::TokenKind::OpenParen: {
      // chama parse_binary_expression com min_precedence = 0

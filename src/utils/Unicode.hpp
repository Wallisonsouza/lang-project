...
public:
  inline static void trim_u32string(std::u32string &s) {
    s.erase(s.begin(),
            std::ranges::find_if(s.begin(), s.end(), [](char32_t ch) {
              return !is_white_space(ch);
            }));
    s.erase(
        std::ranges::find_if(s.rbegin(), s.rend(),
                             [](char32_t ch) { return !is_white_space(ch); })
            .base(),
        s.end());
  }
  ...
  static int char_width(char32_t cp) {
    ...
    if (cat == U_COMBINING_SPACING_MARK || cat == U_NON_SPACING_MARK)
      return 0; // acentos combinantes
    ...
    if ((cp >= 0x1F300 &&
         cp <= 0x1FAFF) || // símbolos, faces, transportes, recentes
  ...
  inline static bool is_letter(char32_t cp) {
    return (cp >= 0x61 && cp <= 0x7A) || (cp >= 0x41 && cp <= 0x5A);
  }

  inline static bool is_digit(char32_t cp) {
    return (cp >= 0x30 && cp <= 0x39);
  }

  inline static bool is_alphanumeric(char32_t cp) {
  ...
  inline static bool is_symbol(char32_t cp) {
    ...
    return cat == U_MATH_SYMBOL || cat == U_CURRENCY_SYMBOL ||
           cat == U_MODIFIER_SYMBOL || cat == U_OTHER_SYMBOL;
  }
};

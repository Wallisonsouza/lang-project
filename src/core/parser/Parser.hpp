...
//   Parser(core::LangData &data) : expr_parser(data, expression_plugins), data(data) {}

//   std::shared_ptr<Node> run(core::Stream<core::Token> &stream) {
//     for (auto &p : statement_plugins) {
//       if (auto node = p->match(*this, stream)) return node;
//     }
//     return nullptr;
//   }

//   std::shared_ptr<ExpressionNode> parse_expression(core::Stream<core::Token> &stream) { return expr_parser.parse(stream); }

//   std::vector<std::shared_ptr<Node>> generate_ast(std::vector<std::shared_ptr<core::Token>> &tokens) {

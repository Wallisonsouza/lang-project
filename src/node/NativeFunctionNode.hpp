...
struct NativeFunctionNode : public ASTNode {
  ...
  std::function<LanguagePrimitives(const std::vector<LanguagePrimitives> &)>
      impl;

  explicit NativeFunctionNode(
      std::function<LanguagePrimitives(const std::vector<LanguagePrimitives> &)>
          f)
      : impl(std::move(f)) {}
  ...
  LanguagePrimitives call(const std::vector<LanguagePrimitives> &args) {
    return impl(args);
  }

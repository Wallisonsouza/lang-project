...
struct CallNode : public Node {
  ...
  CallNode(std::string name, std::vector<std::unique_ptr<Node>> arguments) : funcName(std::move(name)), args(std::move(arguments)) {}
  ...
  LanguagePrimitives execute(Environment &env) override {
    ...
    for (std::unique_ptr<Node> &arg : args) {
      argValues.push_back(arg->execute(env));
    }

    std::shared_ptr<Node> node = env.getSymbol(funcName);

...
//   // ---------------------------------------------
//   inline std::shared_ptr<T> current() const noexcept {
//     return has() ? data[pos] : nullptr;
//   }

//   inline std::shared_ptr<T> peek(size_t offset = 0) const noexcept {
//     return has(offset) ? data[pos + offset] : nullptr;
//   }
...
//   // SLICE
//   // ---------------------------------------------
//   inline std::vector<std::shared_ptr<T>> slice(size_t start,
//                                                size_t end) const noexcept {
//     if (start > end || end > data.size())
//       return {};
//     return std::vector<std::shared_ptr<T>>(data.begin() + start,
//                                            data.begin() + end);
//   }
// };
